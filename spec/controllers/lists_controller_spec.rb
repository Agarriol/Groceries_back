...
RSpec.describe ListsController, type: :controller do
    ...
    context 'when user is logged' do
      ...
      describe 'request' do
        ...
        it 'returns correct data' do
          expect(data.first['id']).to eq(@list.id)
          expect(data.first['title']).to eq(@list.title)
          expect(data.first['description']).to eq(@list.description)
          expect(data.first['state']).to eq(@list.state)
          expect(data.first['user_id']).to eq(@list.user_id)
        end
      end
      ...
      describe 'filters' do
        describe 'title' do
          ...
          it 'returns filtered resources' do
            expect(data.size).to eq(1)
            expect(data.map { |x| x['id'] }).to include(@list.id)
          end
        end
      end
      ...
      describe 'order' do
        describe 'title' do
          describe 'asc' do
            ...
            it 'returns ordered resources' do
              expect(data.first['id']).to eq(List.order(title: 'asc').first.id)
            end
          end
          ...
          describe 'desc' do
            ...
            it 'returns ordered resources' do
              expect(data.last['id']).to eq(List.order(title: 'desc').last.id)
            end
          end
        end
        ...
        describe 'created_at' do
          describe 'asc' do
            ...
            it 'returns ordered resources' do
              expect(data.first['id']).to eq(List.order(created_at: 'asc').first.id)
            end
          end
          ...
          describe 'desc' do
            ...
            it 'returns ordered resources' do
              expect(data.last['id']).to eq(List.order(created_at: 'desc').last.id)
            end
          end
        end
      end
  ...
  describe 'POST #create' do # TODO
    ...
    context 'when user is logged' do
      ...
      context 'with incorrect params' do
        ...
        #   expect(data).to eq(ActiveRecord::RecordNotFound)
        # end
      end
    end
  end

  describe 'PUT #update' do # TODO
  ...
  describe 'DELETE #destroy' do # TODO
    context 'when user is not logged' do
      before do
        sign_out
